 ...
 * single-admin tool, callers only ever read the first row via .limit(1),
 * and in practice that read order is stable enough that a duplicate row
 * here is a non-issue, not worth an advisory lock.
 */
 ...
 * no anchor (nothing scheduled yet). Letters land at +7d, +14d, +21d, +28d
 * from that start, at the 6am-Central publish hour, exactly as
 * computeScheduledFor persists them.
 *
