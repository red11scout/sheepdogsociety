...
- **DOM checks are not verification for visual UIs — only pixels are.** The marquee verse card (#80/#81) "verified" twice via isVisible()/innerText while every human saw nothing: an ancestor `mask-image` (nw-lintel's edge fade) discards all paint outside the band's border box, leaving the card hit-testable, opacity-1, in-viewport — and invisible. isVisible() checks box+display, not paint. Verify popovers by reading the screenshot, and portal overlays to <body> so ancestor mask/overflow/transform can never eat them. (Also real: synthetic element.click() skips the tap's focus-then-click order that the :focus-visible guard in #81 handles.)
